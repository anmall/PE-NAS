...
class Cell:
    ...
    def encode_cell_ops_onehot(self):
        """
        compute the "standard" encoding,
        i.e. adjacency matrix + op list encoding
        """
        encoding = np.zeros(  ((NUM_VERTICES ** 2 - NUM_VERTICES) // 2)  )
        n = 0
        dic = {CONV1X1: 0, CONV3X3: 1, MAXPOOL3X3: 2}
        for i in range(NUM_VERTICES - 1):
            for j in range(i+1, NUM_VERTICES):
                encoding[n] = self.matrix[i][j]
                n += 1
        ops_encoding = np.array([dic[self.ops[i]] for i in range(1, NUM_VERTICES - 1)])
        b = np.zeros( (len(ops_encoding), len(OPS)) )
        b[np.arange(len(ops_encoding)), ops_encoding] = 1

        join = np.concatenate( (encoding ,b.flatten())).tolist()
        return join
